import { Tool, Category } from "@/types/tool";

export const categories: Category[] = [
  { id: "recent", name: "Recently Used", icon: "Clock", path: "/category/recent" },
  { id: "favorites", name: "Favorites", icon: "Star", path: "/category/favorites" },
  { id: "text", name: "Text Processing", icon: "Type", path: "/category/text" },
  { id: "converter", name: "Format Converters", icon: "Repeat", path: "/category/converter" },
  { id: "generator", name: "Quick Generators", icon: "Wand2", path: "/category/generator" },
  { id: "calculator", name: "Smart Calculators", icon: "Calculator", path: "/category/calculator" },
  { id: "image", name: "Image Studio", icon: "Image", path: "/category/image" },
  { id: "code", name: "Dev Utilities", icon: "Code", path: "/category/code" },
  { id: "finance", name: "Money Management", icon: "DollarSign", path: "/category/finance" },
  { id: "utility", name: "Helper Tools", icon: "Wrench", path: "/category/utility" },
];

export const tools: Tool[] = [
  {
    id: "case-converter",
    title: "Case Converter",
    description: "Convert text between uppercase, lowercase, title case, and more",
    category: "text",
    badge: "Text",
    badgeVariant: "blue",
    path: "/tools/case-converter",
    icon: "TextCursor",
  },
  {
    id: "word-counter",
    title: "Word Counter",
    description: "Count words, characters, sentences, and paragraphs in your text",
    category: "text",
    badge: "Text",
    badgeVariant: "blue",
    path: "/tools/word-counter",
    icon: "FileText",
  },
  {
    id: "json-formatter",
    title: "JSON Formatter",
    description: "Format and validate JSON data with syntax highlighting",
    category: "code",
    badge: "Developer",
    badgeVariant: "purple",
    path: "/tools/json-formatter",
    icon: "Braces",
  },
  {
    id: "base64-encoder",
    title: "Base64 Encoder/Decoder",
    description: "Encode and decode text to/from Base64 format",
    category: "converter",
    badge: "Converter",
    badgeVariant: "green",
    path: "/tools/base64",
    icon: "Binary",
  },
  {
    id: "uuid-generator",
    title: "UUID Generator",
    description: "Generate unique identifiers (UUIDs) in various formats",
    category: "generator",
    badge: "Generator",
    badgeVariant: "orange",
    path: "/tools/uuid-generator",
    icon: "Hash",
  },
  {
    id: "password-generator",
    title: "Password Generator",
    description: "Create strong, secure passwords with customizable options",
    category: "generator",
    badge: "Generator",
    badgeVariant: "orange",
    path: "/tools/password-generator",
    icon: "Lock",
  },
  {
    id: "color-picker",
    title: "Color Picker",
    description: "Pick colors and convert between HEX, RGB, HSL formats",
    category: "image",
    badge: "Image",
    badgeVariant: "pink",
    path: "/tools/color-picker",
    icon: "Palette",
  },
  {
    id: "qr-generator",
    title: "QR Code Generator",
    description: "Generate QR codes from text, URLs, or other data",
    category: "generator",
    badge: "Generator",
    badgeVariant: "orange",
    path: "/tools/qr-generator",
    icon: "QrCode",
  },
  {
    id: "url-encoder",
    title: "URL Encoder/Decoder",
    description: "Encode and decode URLs and query parameters",
    category: "converter",
    badge: "Converter",
    badgeVariant: "green",
    path: "/tools/url-encoder",
    icon: "Link",
  },
  {
    id: "markdown-preview",
    title: "Markdown Preview",
    description: "Write and preview Markdown in real-time",
    category: "text",
    badge: "Text",
    badgeVariant: "blue",
    path: "/tools/markdown-preview",
    icon: "FileCode",
  },
  {
    id: "hash-generator",
    title: "Hash Generator",
    description: "Generate MD5, SHA-1, SHA-256 hashes from text",
    category: "code",
    badge: "Developer",
    badgeVariant: "purple",
    path: "/tools/hash-generator",
    icon: "Fingerprint",
  },
  {
    id: "timestamp-converter",
    title: "Timestamp Converter",
    description: "Convert between Unix timestamps and human-readable dates",
    category: "converter",
    badge: "Converter",
    badgeVariant: "green",
    path: "/tools/timestamp-converter",
    icon: "Clock",
  },
  {
    id: "regex-tester",
    title: "Regex Tester",
    description: "Test and debug regular expressions with live matching",
    category: "code",
    badge: "Developer",
    badgeVariant: "purple",
    path: "/tools/regex-tester",
    icon: "Search",
  },
  {
    id: "lorem-ipsum",
    title: "Lorem Ipsum Generator",
    description: "Generate placeholder text for your designs",
    category: "generator",
    badge: "Generator",
    badgeVariant: "orange",
    path: "/tools/lorem-ipsum",
    icon: "Text",
  },
  {
    id: "percentage-calculator",
    title: "Percentage Calculator",
    description: "Calculate percentages, discounts, and increases",
    category: "calculator",
    badge: "Calculator",
    badgeVariant: "cyan",
    path: "/tools/percentage-calculator",
    icon: "Percent",
  },
  {
    id: "bmi-calculator",
    title: "BMI Calculator",
    description: "Calculate your Body Mass Index and health status",
    category: "calculator",
    badge: "Calculator",
    badgeVariant: "cyan",
    path: "/tools/bmi-calculator",
    icon: "Activity",
  },
  // Finance Tools
  {
    id: "loan-calculator",
    title: "Loan Calculator",
    description: "Calculate monthly payments and total interest for loans",
    category: "finance",
    badge: "Finance",
    badgeVariant: "green",
    path: "/tools/loan-calculator",
    icon: "DollarSign",
  },
  {
    id: "interest-calculator",
    title: "Interest Calculator",
    description: "Calculate simple and compound interest",
    category: "finance",
    badge: "Finance",
    badgeVariant: "green",
    path: "/tools/interest-calculator",
    icon: "TrendingUp",
  },
  {
    id: "investment-calculator",
    title: "Investment Calculator",
    description: "Estimate investment returns over time",
    category: "finance",
    badge: "Finance",
    badgeVariant: "green",
    path: "/tools/investment-calculator",
    icon: "PiggyBank",
  },
  {
    id: "currency-converter",
    title: "Currency Converter",
    description: "Convert between different currencies with real-time rates",
    category: "finance",
    badge: "Finance",
    badgeVariant: "green",
    path: "/tools/currency-converter",
    icon: "ArrowLeftRight",
  },
  {
    id: "credit-card-calculator",
    title: "Credit Card Calculator",
    description: "Calculate credit card payoff time and total interest",
    category: "finance",
    badge: "Finance",
    badgeVariant: "green",
    path: "/tools/credit-card-calculator",
    icon: "CreditCard",
  },
  // Utility Tools
  {
    id: "unit-converter",
    title: "Unit Converter",
    description: "Convert between different units of measurement",
    category: "utility",
    badge: "Utility",
    badgeVariant: "purple",
    path: "/tools/unit-converter",
    icon: "Ruler",
  },
  {
    id: "age-calculator",
    title: "Age Calculator",
    description: "Calculate your exact age in years, months, and days",
    category: "utility",
    badge: "Utility",
    badgeVariant: "purple",
    path: "/tools/age-calculator",
    icon: "Cake",
  },
  {
    id: "date-calculator",
    title: "Date Calculator",
    description: "Calculate difference between two dates",
    category: "utility",
    badge: "Utility",
    badgeVariant: "purple",
    path: "/tools/date-calculator",
    icon: "Calendar",
  },
  {
    id: "random-picker",
    title: "Random Picker",
    description: "Pick random names or items from a list",
    category: "utility",
    badge: "Utility",
    badgeVariant: "purple",
    path: "/tools/random-picker",
    icon: "Shuffle",
  },
  // Image Tools
  {
    id: "image-compressor",
    title: "Image Compressor",
    description: "Compress images to reduce file size",
    category: "image",
    badge: "Image",
    badgeVariant: "pink",
    path: "/tools/image-compressor",
    icon: "ImageDown",
  },
  {
    id: "image-resizer",
    title: "Image Resizer",
    description: "Resize images to any dimension",
    category: "image",
    badge: "Image",
    badgeVariant: "pink",
    path: "/tools/image-resizer",
    icon: "Maximize2",
  },
  // Text Tools
  {
    id: "text-diff",
    title: "Text Diff Checker",
    description: "Compare two texts and find differences",
    category: "text",
    badge: "Text",
    badgeVariant: "blue",
    path: "/tools/text-diff",
    icon: "GitCompare",
  },
  {
    id: "text-sorter",
    title: "Text Sorter",
    description: "Sort text lines alphabetically",
    category: "text",
    badge: "Text",
    badgeVariant: "blue",
    path: "/tools/text-sorter",
    icon: "ArrowDownAZ",
  },
  // Converter Tools
  {
    id: "html-to-text",
    title: "HTML to Text",
    description: "Convert HTML to plain text",
    category: "converter",
    badge: "Converter",
    badgeVariant: "green",
    path: "/tools/html-to-text",
    icon: "FileCode",
  },
  {
    id: "csv-to-json",
    title: "CSV to JSON",
    description: "Convert CSV data to JSON format",
    category: "converter",
    badge: "Converter",
    badgeVariant: "green",
    path: "/tools/csv-to-json",
    icon: "FileJson",
  },
];
